...
class EmeraldSourceWriter:
    ...
    def write_field_items(self, orig_items: list, rand_items: list):
        """Patch finditem ITEM_X lines in map scripts."""
        ...
        for orig, rand in zip(orig_items, rand_items):
            ...
            if out_abs not in file_patches:
                file_patches[out_abs] = []
            file_patches[out_abs].append((orig.line_index, rand.item_const))

        patched = 0
        for out_abs, patches in file_patches.items():
            lines = self._get_lines(out_abs)
            for li, new_item in patches:
                if li >= len(lines):
                    continue
                m = finditem_re.match(lines[li])
                if m:
...
"""
    ...
    def write_starting_items(self, bag_items: list, pc_items: list):
        ...
        body = []
        for it in bag:
            body.append("    AddBagItem(%s, %d);" % (it["const"], _clamp(it.get("qty", 1))))
        for it in pc:
            body.append("    AddPCItem(%s, %d);" % (it["const"], _clamp(it.get("qty", 1))))
